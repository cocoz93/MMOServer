...
int main()
{
    ...
    while (true)
    {
        ...
        // 1초 주기 통계 출력
        int connected = g_Stats.connectedCount.load(std::memory_order_relaxed);
        int ready     = g_Stats.readyCount.load(std::memory_order_relaxed);

        int64_t curRecv = g_Stats.recvPackets.load(std::memory_order_relaxed);
        int64_t curSend = g_Stats.sendPackets.load(std::memory_order_relaxed);
        int64_t recvPps = curRecv - prevRecvPkts;
        int64_t sendPps = curSend - prevSendPkts;
        prevRecvPkts = curRecv;
        prevSendPkts = curSend;

        int64_t err = g_Stats.disconnectFromServer.load(std::memory_order_relaxed);
        ...
        if (rttSamples > 0)
        {
            ...
            {
                cum += g_Stats.rttBuckets[i].load(std::memory_order_relaxed);
