...
- [x] **2-D** TSan / ASan — **완료 (2026-08-03). 선택 페이즈였지만 실행했다**
  ...
  - **함정**: TSan은 WSL2에서 `FATAL: unexpected memory mapping`으로 **시작조차 못 한다**(ASLR 충돌). 이때 **종료 코드는 66**인데, `setarch -R`로 제대로 띄워 경합을 찾았을 때도 **같은 66**이라 rc만으로는 두 상황이 구분되지 않는다 (2026-08-04 문서감사 A3에서 5회 재현 실측 — 그 전 "rc=0" 기록은 틀렸다. `TSAN_OPTIONS=exitcode=0`을 준 상태였을 가능성) → **`setarch -R`로 ASLR을 끄고, 출력 첫 줄까지 볼 것**. 처음 3개 테스트를 이 상태로 "돌렸다"로 셀 뻔했다
