...
- ~~관측대기 1건: 리눅스 서버가 SIGTERM 뒤 2초 내 미종료(SIGKILL로 정리함)~~ → **재현·수정 완료 (2026-08-21)**. 결함 2건이 사슬로 물려 있었다
  - **SIGTERM 무한 행** — 리눅스 `close`는 다른 스레드가 블록 중인 `accept`를 깨우지 못한다(진행 중인 호출이 파일 객체를 붙들어 통지가 안 감). 연결이 더 오지 않으면 `_acceptThread.join()`에서 영구 대기. **수정**: 리눅스는 `shutdown(listen, SHUT_RDWR)`로 깨우고 `close`는 join 뒤로. **A/B 실측**: 원본 = 20초 초과 행(`wchan=inet_csk_accept`·main은 futex) / 수정본 = **1,015ms 정상 종료**. 최소 재현으로 기전까지 확인 — `close`는 3초 뒤에도 안 깨우고 `shutdown`은 `accept`를 **errno 22(EINVAL)**로 깨운다
  - **미등록 세션 IOCount 언더플로** — epoll 팔은 완료 통지가 없어 `TransportRequestDisconnect`가 등록 ref를 직접 감소시키는데, 등록한 적 없는 세션(attach 실패분·종료 루프의 미접속분)에도 무조건 실행돼 0→−1. **수정**: `_epollRegistered` 표식을 집어내며 지워 등록분만 한 번 감소. 표식은 `epoll_ctl(ADD)` **전에** 세운다(성공 후에 세우면 등록 직후 통지와 경합해 ref가 안 풀림 → 좀비 세션). 선언부 초기화 필수 — 미접속 세션은 `Initialize`를 안 타므로 리셋에만 맡기면 쓰레기 값을 읽는다
  - **행은 조건부였다 — 들어오는 연결이 없을 때만.** `close`는 블록된 `accept`가 파일 객체를 붙들고 있어 소켓을 실제로 해제하지 못한다. **소켓이 계속 LISTEN 상태로 남아**, 새 연결 하나가 오면 `accept`가 깨어나고 그때는 종료가 세션 정리 루프까지 흘러 언더플로가 그대로 찍힌다. 그래서 로그에 「행만」 남기도 하고 「언더플로만」 남기도 한다 — 실측 3종이 한 산식으로 맞물린다
    - 연결 없음 → **20초 초과 행, 언더플로 0건**(루프에 도달 못 함)
    - SIGTERM 2초 뒤 새 연결 1회 → **즉시 종료, 언더플로 5,996건 = 6000 − 4**(접속 3 + 깨운 연결 1)
    - 08-04 로그 → **5,990건 = 6000 − 10**(그날 접속 10명). `MaxClients: 6000`은 같은 로그 기동부에서 확인
    - 세 경우 모두 **미접속 세션 수와 정확히 일치** — 원인이 「등록한 적 없는 세션에서의 감소」임을 수치로 확정
  - 검증: Windows Release IOCP·RIO **경고0 오류0** / 리눅스 빌드 기준선 경고만 / 수정본 종료 시 언더플로 **0건**

### epoll 팔 결함 감사 — 6건 전부 수정 (2026-08-23)

4단계는 "정확성 졸업"으로 닫혀 있었지만, 새 diff(병합본 → HEAD) 감사에서 **동시성 결함 6건**이 나왔다.
위 SIGTERM·언더플로 2건(F5·F6)이 그중 둘이고, 나머지 넷은 아래. **전부 `Transport_Epoll.cpp` 국소 수정**이라
Windows 팔은 컴파일조차 되지 않는 코드다(`#ifndef _WIN32`).

- **F1 수신 동시 접근 (치명, `c7f4afd`)** — level-trigger는 "커널 버퍼에 데이터가 남았나"만 보고 알려서,
  앞 워커가 읽는 중에도 다른 워커에게 같은 소켓을 또 준다. 잠금 없는 `_recvQ`(`CRingBufferST`)를 둘이 동시에 밀었다.
  IOCP는 recv를 세션당 하나만 걸어 완료도 워커 하나에만 가므로 이 문제가 구조적으로 없다.
  **수정**: 세션별 수신 게이트(`_recvBusy`, CAS). 물러난 통지는 흘려도 안전 — level-trigger가 다음 라운드에 다시 준다.
  **TSan A/B 실측**(게이트만 차이나게): 게이트 전 = 두 워커가 `MoveWritePtr`에 동시 진입(링버퍼 프레임 34·`recv()` 동시호출 1),
  게이트 후 = **0건, 3회 재현**.
  ⚠ **비용**: 물러난 워커가 같은 통지를 곧 다시 받아 헛돈다. 소규모 부하에서 `mmo_recv_contention_total`이
  수신 syscall 수와 **거의 1:1**(436 vs 442)이었다 — 절대량은 작았지만 실부하 비율은 5-B에서 확인할 것.
  **임시 조치다** — 워커별 epoll(`SO_REUSEPORT`)로 세션 소유를 고정하면 게이트째 사라진다
- **F2 ref 반환 순서 역전 (`c0c8b12`)** — `writev` 실패 경로에서 `IOCountDecrement`가 `RequestDisconnectSession`보다
  앞서, 그 감소가 마지막 ref였으면 세션이 해제·재사용된 뒤 **새 접속을 끊는** 창이 있었다.
  순서를 뒤집어 IOCP 팔 `PostSend`와 통일(끊기 먼저 → 감소 나중)
- **F3 EPOLLOUT 표식 반영이 잠금 밖 (`ec5dd39`)** — `_epollWantWrite` 갱신이 제출 잠금 해제 뒤라, 게임스레드와
  워커의 계산·반영 순서가 뒤집히면 **잔여가 남았는데 EPOLLOUT이 꺼진** 채 멈출 수 있었다(헤더 불변식 위반).
  반영을 잠금 안으로. 함께 「잠금 중 enqueue하고 튕긴 데이터를 아무도 안 보내는」 구멍도 닫음
  (빈 링으로 끝났을 때만 해제 후 재확인 → 남았으면 한 라운드 더)
- **F4 RDHUP를 수신보다 먼저 처리 (`294d5cf`)** — "보내고 바로 닫는" 상대의 데이터와 FIN이 한 이벤트로 오는데
  (`EPOLLIN|EPOLLRDHUP`), RDHUP를 보자마자 끊어 **마지막 패킷을 버렸다**. 파손(HUP·ERR)만 즉시 끊고
  RDHUP는 수신 경로로 넘겨 `recv()==0` 자리가 끊게 함(IOCP와 같은 의미론).
  실측 = 수정 전 5회 전부 유실 → **수정 후 워커 1개에서 10/10 회수**. 워커 2개에서 난 1건 실패는 격리 결과 F1(수신 직렬화) 소관이었다
  — **F1 게이트를 넣은 지금 WT4로 10/10 재확인이 남아 있다**(5-B 때 같이 볼 것)

**클라 3곳 빌드 불가도 함께 해소 (`5bb359e`)** — 5-0의 타입 교체(`wchar_t`→`ChatChar`) 때 서버만 정리해,
`DummyClient.cpp`·`ClientNetwork.cpp`·`GameInstance.cpp`가 `wchar_t` 전용 CRT 함수(`wcscpy_s`·`wmemcpy`)를
호출한 채 남아 **8/21부터 컴파일이 막혀 있었다**(와이어는 Windows에서 폭이 같아 멀쩡 → 컴파일에서만 걸림).
폭을 가정하지 않고 경계에서 원소별 변환. **5-C에 쓰는 부하 클라라, 이걸 고치기 전에는 측정 진입 자체가 불가능했다.**

검증: Windows 4개 솔루션 **경고0 오류0** / 리눅스 경고0 / **Windows 더미 10명 → 리눅스 서버 채팅 왕복 158회**
(RTT 평균 22ms·패킷에러 0·parseFail 0) / SIGTERM 종료 713ms·언더플로 0
...
- [x] **2-D** TSan / ASan — **완료 (2026-08-03). 선택 페이즈였지만 실행했다**
  ...
  - **함정**: TSan은 WSL2에서 `FATAL: unexpected memory mapping`으로 **대개 시작조차 못 한다**(ASLR 충돌). **결과가 실행마다 갈린다** — 20회 실측(2026-08-05 자가검증 B2)에서 `rc=66`·못 뜸 **16회** / `rc=66`·**정상 실행 2회** / `rc=139`(SIGSEGV) **2회**. `setarch -R`로 띄우면 `rc=66`·race 5로 **일정**하다. 즉 **rc=66이 "못 떴다"와 "떴는데 경합을 찾았다"를 모두 가리키고, 실패 코드마저 66/139로 갈린다** → **`setarch -R`로 ASLR을 끄고, 출력 첫 줄까지 볼 것**. 처음 3개 테스트를 이 상태로 "돌렸다"로 셀 뻔했다 (그 전 "rc=0" 기록은 틀렸다 — `TSAN_OPTIONS=exitcode=0`을 준 상태였을 가능성)
...
> **⏸ 여기서부터는 측정 작업이다 — PC가 한가할 때 재개할 것 (2026-08-04 중단)**
>
> 남은 페이즈(5-B·5-C·4-J·6-B)는 전부 수치를 재는 일이라, 다른 작업으로 PC가 바쁘면
> tick p99·CPU 점유율이 그 노이즈를 그대로 먹는다. **측정값이 의미를 잃으므로 미룬다.**
>
...
- [x] **5-A** WSL2 포트포워딩 — **완료 (2026-08-04). 설정이 필요 없었다**
  - **판정 결과**: Windows 호스트에서 `127.0.0.1:6000`·`localhost:6000` **연결 성공**, 모니터 `9090/metrics`도 HTTP 200
  - 실제 부하 클라(`MMOStressClient.exe`, `ServerIp=127.0.0.1`)를 10명으로 붙여 확인

    | 지표 | 값 |
    |---|---|
    | `mmo_session_count` | **10** (정확히 접속 수) |
    | `mmo_recv_packets_total` | 797 (실제 게임 패킷 왕래) |

  > **계획서 전제가 틀렸다** — "기본 NAT라 외부에서 안 붙는다"고 적어 뒀지만, WSL2는 `localhostForwarding`이 기본 켜져 있어 **같은 PC의 Windows 쪽에서는 `localhost`로 그냥 붙는다.** 별도 `netsh portproxy`가 필요 없었다.
  > 정확히 하면: **다른 PC에서** 붙이려면 여전히 설정이 필요하다(WSL IP는 재부팅마다 바뀌므로 portproxy + 방화벽). 5단계는 같은 PC에서 부하를 주므로 해당 없다.

  - **덤으로 5-0이 실증됐다** — 부하 클라는 Windows에서 빌드된 기존 실행 파일인데, 리눅스 서버와 패킷이 정상적으로 오갔다. `char16_t` 교체로 와이어 포맷이 안 바뀌었다는 근거다
...
> **⚠ 전제가 바뀌었다 (2026-08-23 결정).** 원래는 「같은 골격 위에서 전송계층만 교체해 비교」였다.
> 그런데 **지금 골격은 중립이 아니라 IOCP에 맞춰 만든 것**이다(단일 accept 스레드·세션 공유 풀·1-pending 전제).
> 그 위에 epoll을 얹은 비교는 모델 우열이 아니라 「골격이 누구 편인지」를 재게 된다.
> 실무 질문도 "같은 골격에서 어느 API가 빠른가"가 아니라 **"각자 제대로 짜면 어디까지 가는가"**다.
>
> **→ 각자 최선끼리 비교한다.** 구조만 각자 정석으로 두고, 그 위 조건(게임로직·맵·부하 프로파일·코어 예산)은 동일하게.
>
> **6단계 전 선행 작업 = epoll 팔 재설계** (대형 라이브의 정석: nginx·Envoy·Netty가 쓰는 구조)
> - `SO_REUSEPORT` + **워커별 epoll** → 세션이 평생 한 워커 소유(shared-nothing)
> - 그 결과 **F1 수신 게이트가 구조적으로 불필요해진다** — 게이트도 헛돎(`mmo_recv_contention_total`)도 사라짐
> - 바뀌는 것은 `Transport_Epoll.cpp`의 이벤트 루프 하나. 게임로직·세션수명·링버퍼·모니터는 그대로 두고,
>   IOCP 팔은 **손대지 않는다**(검증된 Windows 실측 사슬을 지킨다)
> - 순서: **5-B·5-C 측정(현행 구조) → 재설계 → 6단계**. 5-B·5-C는 버리는 작업이 아니라 재설계 전후 대조군이 된다
