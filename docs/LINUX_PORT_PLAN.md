...
- [x] **2-D** TSan / ASan — **완료 (2026-08-03). 선택 페이즈였지만 실행했다**
  ...
  - **함정**: TSan은 WSL2에서 `FATAL: unexpected memory mapping`으로 **시작조차 못 한다**(ASLR 충돌). 이때도 **종료 코드가 0**이라 "실행됐는데 깨끗하다"로 오해하기 쉽다 (2026-08-04 재확인 — 옛 기록의 rc=66은 실측과 다르다. rc=0이라 더 위험하다) → **`setarch -R`로 ASLR을 끄고 돌릴 것**. 처음 3개 테스트를 이 상태로 "통과"로 셀 뻔했다
