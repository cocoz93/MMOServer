...
powershell -Command "(Get-Content '%~dp0ServerConfig.ini') -replace '^Mode=.*', 'Mode=GameServer' -replace '^MonitorEnabled=.*', 'MonitorEnabled=0' | Set-Content '%~dp0ServerConfig.ini'"
...
start "" /D "%~dp0" IOCP_MMOClient.exe
echo   - Client started
echo.
